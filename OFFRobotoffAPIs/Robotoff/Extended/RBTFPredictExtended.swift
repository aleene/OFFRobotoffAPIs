...
// This file is needed if the Robotoff Predict Endpoint with enumerated types and OFFBarcode type will be used. The file RBTFPredict.swift is also required
...
class RBTFPredictRequestExtended : RBTFPredictRequest {
...
*/
    func RBTFPredictCategoriesProductExtended(barcode: OFFBarcode, deepestOnly: Bool?, threshold: Double?, predictors: [RBTF.Predictors]?, completion: @escaping (_ result: Result<RBTF.PredictResponse, RBTFError>) -> Void) {
        guard barcode.isCorrect else {             completion(.failure(.barcodeInvalid))
            return
        }
        let request = RBTFPredictRequestExtended(barcode: barcode, deepestOnly: deepestOnly, threshold: threshold, predictors: predictors)
